...
Every screenshot is captured from `dev/harness.html` served by `npm run
harness`, in headless Chrome at a device scale factor of 2, with `?shot=1`
hiding the switch panel and `?scene=` arranging the state — see the end of
`dev/harness.js`. The Azure Maps answers are `dev/host.js`'s stand-in over
`dev/fixture.js`; no real key was involved.

| File | Window | Query |
| --- | --- | --- |
| `screenshot.png` | 440×240 | `?shot=1&scene=suggestions` — "1 micro" typed, two suggestions open |
| `screenshot-filled.png` | 440×110 | `?shot=1&scene=filled` — the first suggestion picked; the other columns under the field |
| `screenshot-refused.png` | 440×120 | `?shot=1&scene=refused` — a 403, the CORS sentence under the field |
| `screenshot-nokey.png` | 440×120 | `?shot=1&scene=nokey` — the unconfigured control |

```bash
chrome --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=2 \
  --virtual-time-budget=6000 --window-size=440,240 --screenshot=media/screenshot.png \
  "http://localhost:8282/dev/harness.html?shot=1&scene=suggestions"
```

The logo is a house with the door as the hole; `logo.svg`'s comment records
