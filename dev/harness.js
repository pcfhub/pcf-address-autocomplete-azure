...
(function () {
    ...
    /** The platform's copies of the six other columns. */
    var columns = { city: null, stateOrProvince: null, postalCode: null, country: null, latitude: null, longitude: null };

    function mapsOptions() {
        var choice = document.getElementById('harness-maps').value;

        return {
            status: /^\d+$/.test(choice) ? Number(choice) : 200,
            empty: choice === 'empty',
    ...
    function options() {
        return {
            ...
            value: columnValue,
            columns: columns,
            key: document.getElementById('harness-key').value,
            maps: mapsOptions(),
            inputs: { resolveCoordinates: document.getElementById('harness-coordinates').checked },
    ...
    function notifyOutputChanged() {
        ...
        window.setTimeout(function () {
            ...
            if (Object.prototype.hasOwnProperty.call(outputs, 'addressLine1') && outputs.addressLine1 !== undefined) {
                columnValue = outputs.addressLine1;
                document.getElementById('harness-value').value = outputs.addressLine1 === null ? '' : String(outputs.addressLine1);
            }

            // The platform applies every key the control emitted and leaves
            // the rest alone — which is exactly the distinction `getOutputs`
            // is careful about.
            Object.keys(columns).forEach(function (name) {
                if (Object.prototype.hasOwnProperty.call(outputs, name) && outputs[name] !== undefined) {
                    columns[name] = outputs[name];
                }
            });

            document.getElementById('harness-columns').textContent = JSON.stringify(columns, null, 2);
    ...
    window.__harnessStart = function () {
        ...
        /*
         * Scenes, for a *captured* picture rather than for a person: headless
         * Chrome cannot click, so the states worth photographing arrange
         * themselves. `?shot=1` hides everything but the form; `?scene=` is
         * one of `suggestions` (the list open on "1 micro"), `filled` (the
         * first suggestion picked), `refused` (a 403 under the field) and
         * `nokey` (the unconfigured control). Typing goes through the
         * prototype setter, because React tracks the previous value on the
         * node and swallows a plain assignment as "no change".
         */
        ...
        if (scene) {
            var typeInto = function (text) {
                var input = document.querySelector('.AddressAutocompleteAzure input[role=combobox]');
            ...
            window.setTimeout(function () {
                if (scene === 'nokey') {
                    document.getElementById('harness-key').value = '';
                    render();
                ...
                if (scene === 'refused') {
                    document.getElementById('harness-maps').value = '403';
                    render();
                }
                ...
                window.setTimeout(function () {
                    ...
                    if (scene === 'filled') {
                        window.setTimeout(function () {
                            var option = document.querySelector('.AddressAutocompleteAzure-listbox [role=option]');
