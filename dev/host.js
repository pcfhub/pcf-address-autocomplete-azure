...
(function (root, factory) {
     ...
     *
     * The keys are the ones in `strings/AddressAutocompleteAzure.1033.resx`, and a key that
     ...
     */
    var STRINGS = {
        AddressAutocompleteAzure_Name: 'Address Autocomplete (Azure Maps)',
        AddressAutocompleteAzure_NoAccess: 'You do not have access to this value.',
        AddressAutocompleteAzure_Placeholder: 'Start typing an address',
        AddressAutocompleteAzure_Searching: 'Searching…',
        AddressAutocompleteAzure_NoMatches: 'No addresses match',
        AddressAutocompleteAzure_TypeMore: 'Type at least {0} characters',
        AddressAutocompleteAzure_NoKey: 'No Azure Maps key is set on this control, so it cannot suggest addresses.',
        AddressAutocompleteAzure_Clear: 'Clear address',
        AddressAutocompleteAzure_KeyRefused: 'Azure Maps refused the key.',
        AddressAutocompleteAzure_OriginRefused: "Azure Maps refused this site. Allow it in the account's CORS settings.",
        AddressAutocompleteAzure_RateLimited: 'Azure Maps is rate limiting requests; try again shortly.',
        AddressAutocompleteAzure_ServiceError: 'Azure Maps returned an error ({0}).',
        AddressAutocompleteAzure_Unreachable: 'Azure Maps could not be reached.',
        AddressAutocompleteAzure_TimedOut: 'Azure Maps did not answer in time.',
        AddressAutocompleteAzure_NoCoordinates: 'The address was saved, but Azure Maps returned no coordinates for it.',
    };
    ...
    var DEFAULTS = {
        ...
        /**
         * The Azure Maps subscription key the control is configured with.
         * A string by default so the default mount can search; `''` is the
         * unconfigured control, which must make no request at all. Overridden
         * by `inputs.subscriptionKey` like any other input.
         */
        key: 'rig-subscription-key',

        /**
         * The six optional bound columns, as the platform holds them. `null`
         * is both "mapped and empty" and "never mapped" — the two are one
         * value from inside a control, which is the whole reason `getOutputs`
         * emits a key only for a column it has written. Each may carry its
         * own field-level security in `columnSecurity`, keyed the same way,
         * because a user denied one address column is not denied the street.
         */
        columns: {
            city: null,
         ...
         * refused key, 403 for an origin the account's CORS rule does not
         * name, 429 for rate limiting, 500 for the rest — and none of those
         * is a `fetch` failure; `unreachable` is (the promise rejects, which
         * is a network that never answered), and `hang` never settles at all,
         * which is what the control's own timeout exists for. `empty` is the
         * legitimate HTTP 200 with no features, which the API documents as
         * "no match" rather than an error. `delay` is milliseconds under the
         * fake clock, for ordering two requests.
         */
        maps: {
            status: 200,
            empty: false,
     ...
     */
    /**
     * The Azure Maps stand-in: a `fetch` that answers the two URLs the control
     * builds and delegates everything else to whatever `fetch` was there.
     *
     * **Installed on the global, once per context**, because that is the only
     * seam a control has — `fetch` is not on `context`. The latest context's
     * `maps` options win, so a suite that mounts a 403 host after a 200 one
     * gets the 403. Every request is logged with its URL and the *names* of
     * its headers; the key's value is deliberately not logged, and the URL
     * is asserted to not contain it.
     *
     * The autocomplete answer is the fixture's rows whose formatted address
     * contains the query, in the reference's GeoJSON shape with
     * `geometry: null` — which is what the reference sample carries, and the
     * reason the control makes a second call for coordinates. The geocode
     * answer is the one fixture row matching the query, with its position.
     */
    /**
     * Every Azure Maps request the stand-in has seen, in order, as
     * `{ url, headers, keyed }` — the header *names* and whether the key
     * header carried a value, never the value. Kept here rather than only on
     * a context's call log because a request is made from a callback long
     * after the context that handed the callback down, and possibly after
     * several newer contexts; a suite reads it by position.
     */
    var MAPS_REQUESTS = [];

    var AUTOCOMPLETE = /\/geocode:autocomplete\?/;
    var GEOCODE = /\/geocode\?/;

    function installMapsFetch(o, log) {
        var scope = typeof globalThis !== 'undefined' ? globalThis : window;

        if (!scope.__mapsFetchInstalled) {
            scope.__mapsPrevious = scope.fetch;
            scope.__mapsFetchInstalled = true;
        }

        scope.fetch = function (url, init) {
            var address = String(url);

            if (!AUTOCOMPLETE.test(address) && !GEOCODE.test(address)) {
                return scope.__mapsPrevious
                    ? scope.__mapsPrevious.call(scope, url, init)
            ...
            var record = {
                ...
                keyed: typeof headers['subscription-key'] === 'string' && headers['subscription-key'] !== '',
            };

            MAPS_REQUESTS.push(record);
            log('fetch', record);
            ...
            return new Promise(function (resolve, reject) {
                ...
                var answer = function () {
                    ...
                    if (maps.status !== 200) {
                        resolve(response(maps.status, { error: { code: String(maps.status) } }));

                        return;
                    }

                    var query = decodeURIComponent((address.match(/[?&]query=([^&]*)/) || ['', ''])[1].replace(/\+/g, ' '));
                    var rows = maps.empty ? [] : matches(query, AUTOCOMPLETE.test(address) ? 5 : 1);

                    // A geocode of an address the service cannot place is an
                    // empty collection, not a feature with no geometry.
                    if (GEOCODE.test(address)) {
                        rows = rows.filter(function (row) { return row.position !== null; });
                    }
                    var top = Number((address.match(/[?&]top=(\d+)/) || ['', '5'])[1]);

                    resolve(response(200, {
                        type: 'FeatureCollection',
                        features: rows.slice(0, top).map(function (row) {
                            return AUTOCOMPLETE.test(address) ? autocompleteFeature(row) : geocodeFeature(row);
                        }),
                    }));
                };

                if (maps.delay > 0) {
                    setTimeout(answer, maps.delay);
                } else {
                    answer();
                }
            });
    ...
    /** Fixture rows whose formatted address contains the query, case-insensitively. */
    function matches(query, limit) {
        var needle = query.trim().toLowerCase();

        return (typeof __pcfAddresses !== 'undefined' ? __pcfAddresses : require('./fixture.js'))
            .filter(function (row) {
                return needle !== '' && row.formattedAddress.toLowerCase().indexOf(needle) !== -1;
            })
            .slice(0, limit);
    }

    /** The reference's autocomplete feature: a parsed address and NO geometry. */
    function autocompleteFeature(row) {
        return {
            type: 'Feature',
            geometry: null,
            properties: {
                ...
                address: {
                    ...
                    locality: row.city,
                    adminDistricts: row.region ? [{ name: row.region.name, shortName: row.region.shortName }] : [],
                    postalCode: row.postalCode,
                    countryRegion: { ISO: row.countryIso, name: row.countryName },
    ...
    /** The geocoding feature: the same address with a position, `[lon, lat]`. */
    function geocodeFeature(row) {
        return {
            type: 'Feature',
            geometry: row.position ? { type: 'Point', coordinates: [row.position.longitude, row.position.latitude] } : null,
            properties: {
                type: 'Address',
                confidence: 'High',
                address: autocompleteFeature(row).properties.address,
            },
        };
    }
    ...
    function createContext(options) {
        ...
        /**
         * One of the six optional bound columns, with its own security.
        ...
        function column(name, type) {
            return {
                ...
                attributes: host.publishesMetadata ? { LogicalName: name.toLowerCase() } : undefined,
                type: type,
            };
        }

        installMapsFetch(o, log);
        ...
        return {
            ...
            parameters: Object.assign({
                city: column('city', 'SingleLine.Text'),
                stateOrProvince: column('stateOrProvince', 'SingleLine.Text'),
                postalCode: column('postalCode', 'SingleLine.Text'),
                country: column('country', 'SingleLine.Text'),
                latitude: column('latitude', 'FP'),
                longitude: column('longitude', 'FP'),
                subscriptionKey: { raw: o.key, type: 'SingleLine.Text' },
                // Unset, as a fresh install is: the platform hands an enum's
                // default-value down as its raw, and the rest as null.
                endpoint: { raw: 'public', type: 'Enum' },
                countryRegion: { raw: null, type: 'SingleLine.Text' },
                biasLatitude: { raw: null, type: 'FP' },
                biasLongitude: { raw: null, type: 'FP' },
                regionFormat: { raw: 'short', type: 'Enum' },
                countryFormat: { raw: 'name', type: 'Enum' },
                resolveCoordinates: { raw: false, type: 'TwoOptions' },
                minCharacters: { raw: null, type: 'Whole.None' },
                maxSuggestions: { raw: null, type: 'Whole.None' },
                addressLine1: {
                    raw: o.value,
                    /*
     ...
     *
     * `pcf-scripts` emits `registerControl('PCFHub.AddressAutocompleteAzure', ctor)`
